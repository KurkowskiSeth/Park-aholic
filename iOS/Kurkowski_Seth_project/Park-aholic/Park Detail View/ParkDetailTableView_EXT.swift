...
extension ParkDetail_ViewController {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == 0 {
            ...
            return (parkData?.commentsArray.count)!
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "userCommentsCell", for: indexPath) as? userComments_TableViewCell
            cell?.isUserInteractionEnabled = false
            let commentIDNum = indexPath.row
            let comment = parkData?.commentsArray[commentIDNum]
